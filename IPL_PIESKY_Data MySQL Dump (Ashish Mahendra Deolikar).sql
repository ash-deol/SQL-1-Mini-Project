...
select ibp.BIDDER_ID,BID_STATUS,(count(*)/NO_OF_BIDS)*100 Winning_Percentage
from ipl_bidder_points ibp 
inner join ipl_bidding_details ibd
on ibp.bidder_id=ibd.bidder_id
where bid_status like 'WON'
group by ibp.bidder_id
order by bidder_id;

--  Q2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
SELECT ims.STADIUM_ID,STADIUM_NAME,CITY,count(*) No_of_Matches
...
add column TOSS_WINNER_ID int(10);
update ipl_match
set TOSS_WINNER_ID= if(TOSS_WINNER=1,TEAM_ID1,TEAM_ID2);
alter table ipl_match
add column MATCH_WINNER_ID int(10);
update ipl_match
set MATCH_WINNER_ID= if(MATCH_WINNER=1,TEAM_ID1,TEAM_ID2);

select MATCH_WINNER_ID,TEAM_NAME, round(count(*)/120*100,2) Percentage_of_Wins
from ipl_match im
inner join ipl_team it
on it.TEAM_ID=im.MATCH_WINNER_ID
where WIN_DETAILS like '%won%'
group by MATCH_WINNER_ID
order by MATCH_WINNER_ID;

-- Q4. Show the total bids along with bid team and team name.
select BID_TEAM,TEAM_NAME,sum(NO_OF_BIDS) Total_Bids
from ipl_bidder_points ibp
inner join ipl_bidding_details ibd
on ibp.BIDDER_ID=ibd.BIDDER_ID
join ipl_team it
...
select MATCH_WINNER_ID,TEAM_NAME,WIN_DETAILS
from ipl_match im
inner join ipl_team it
on it.team_id=im.match_winner_id
where WIN_DETAILS like '%won%';
...
alter table ipl_match
add column MATCH_LOST_ID int(10);
update ipl_match
set MATCH_LOST_ID= if(MATCH_WINNER=1,TEAM_ID2,TEAM_ID1);

select TEAM_NAME,count(*) Matches_Won
from ipl_match im
inner join ipl_team it
...
select TEAM_NAME,count(*) Matches_Lost
from ipl_match im
inner join ipl_team it
on team_id=MATCH_LOST_ID
group by TEAM_NAME
order by team_id;

-- Q7. Display the bowlers for Mumbai Indians team.
select itp.PLAYER_ID,PLAYER_NAME
from ipl_team_players itp
inner join ipl_player ip
on itp.PLAYER_ID=ip.PLAYER_ID
where PLAYER_ROLE like 'Bowler' and itp.Remarks like 'TEAM - MI';

-- Q8. How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order.
select itp.TEAM_ID,TEAM_NAME,count(PLAYER_ROLE) Allrounder_Player
from ipl_team_players itp
inner join ipl_player ip
on itp.PLAYER_ID=ip.PLAYER_ID
inner join ipl_team it
